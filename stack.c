...
void print_stack(Stack stack){
    for (int i =0;i<=stack->comprimento;i++){
        switch(stack->elemento[i].tipo){
            case STACK_CHAR:
                printf("%c",stack->elemento[i].data.val_c);break;
            case STACK_LONG:
                printf("%ld",stack->elemento[i].data.val_l);break;
            case STACK_DOUBLE:
                printf("%g",stack->elemento[i].data.val_d);break;
            case STACK_STRING:
                printf("%s",stack->elemento[i].data.val_s);break;
            default:
                fprintf(stderr,"erro no tipo do print_stack");exit(EXIT_FAILURE);
        }
    }
    printf("\n");
}